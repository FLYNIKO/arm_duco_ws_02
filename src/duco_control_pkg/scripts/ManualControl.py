...
class system_control:
    def __init__(self, duco_cobot, app=None):
        ...
        self.car_state = [8, 2] # 第一个是车辆启动状态，2-停车，8-开车 | 第二个是喷涂机喷涂状态，2-停喷，8-开喷
    ...
    def ob_cross_check(self, radar, direction, delay):
        ...
        # 初始化突变检测相关变量
        prev_distance = self.get_distance(radar, direction)
        if prev_distance >= 2:
            prev_distance = 2.0
        jump_count = 0
        # 开车停喷
        ...
        while self.ob_sidemotive_flag:
            # 获取当前距离
            current_distance = self.get_distance(radar, direction)
        ...
        self.duco_ob.movel([self.init_pos[0], self.init_pos[1], origin_pos[2], self.init_pos[3], self.init_pos[4], self.init_pos[5]], self.ob_vel, self.ob_acc, 0, '', '', '', True)
                ...
                if self.is_obstacle_detected():
                    ...
                    if self.ob_status == 0: # 全向避障
                        ...
                        if self.paint_motion == 1 or self.paint_motion == 5 or self.paint_motion == 2 or self.paint_motion == 4 or self.paint_motion == 3 or self.paint_motion == 6:
                            ...
                            if ob_data['left_rear'] and self.car_direction == 0:
                                ...
                                self.ob_cross_check("right", "up", pause_time)
                            ...
                            if ob_data['right_rear'] and self.car_direction == 1:
                                pause_time = 0.4 / self.car_speed
                                self.duco_ob.stop(True)
                                self.ob_flag = True
                                ob_data = self.get_obstacle_status()
                                ...
                                self.ob_cross_check("left", "down", pause_time)
    ...
    def get_key_input(self):
        if time.time() - self.last_key_time > KEYTIMEOUT:
            ...
            self.column_overlap = max(-10, min(10, self.latest_keys[17])) / 100
    ...
    def compute_spray_angle(self, column_segment_length, spray_distance=0.55):
        ...
        返回喷嘴旋转角度 (°)
        """
        theta_rad = math.atan(((column_segment_length - self.column_overlap)/ 2) / spray_distance)
    ...
    def _column_paint_one_round(self, paint_column_num, paint_round_num,
                                last_paint_height, last_lift_height,
                                min_arm_z, lift_down_dist):
        arm_paint_column_list = []
        for i in range(self.arm_segment_num):
            self.arm_column_right = [self.arm_column_right_x, self.arm_column_right_y,
                                     self.arm_column_right_z - (i * self.column_segment_length),
                                     self.init_pos[3], self.init_pos[4], self.init_pos[5]]
            self.arm_column_left  = [self.arm_column_left_x,  self.arm_column_left_y,
                                     self.arm_column_left_z  - (i * self.column_segment_length),
                                     self.init_pos[3], self.init_pos[4], self.init_pos[5]]
            if self.arm_column_left[2] < min_arm_z or self.arm_column_right[2] < min_arm_z:
                continue
            if i % 2 == 1:
        ...
        self.duco_cobot.movel(arm_paint_column_list[0], vel_fast, self.acc, 0, '', '', '', True)
        self.car_state = [2, 8]
        ...
        for i, point in enumerate(arm_paint_column_list[1:], start=1):
            ...
            if i % 2 == 1:
                ...
                vel_use = vel_fast
                acc_use = 0.4
            self.duco_cobot.movel(point, vel_use, acc_use, 0, '', '', '', True)
        ...
        if paint_column_num > 0:
            ...
            return paint_column_num, paint_round_num, last_paint_height, last_lift_height, False
        else:
            rospy.logwarn("---\n方柱喷涂：喷涂方柱完成")
            return paint_column_num, paint_round_num, last_paint_height, last_lift_height, True
    ...
    def auto_paint_sync(self):
        ...
        if self.paint_object == 0:
            ...
            self.esp32_spray_state = 0
            self.car_state = [2, 2]
            lift_down_dist = - abs(self.arm_segment_num * self.column_segment_length)
            min_arm_z = -0.8
            print("\ndown dist = %s\n", lift_down_dist)
            ...
            while self.autopaint_flag:
                ...
                paint_column_num, paint_round_num, last_paint_height, last_lift_height, should_break = \
                    self._column_paint_one_round(paint_column_num, paint_round_num,
                                                 last_paint_height, last_lift_height,
                                                 min_arm_z, lift_down_dist)
                if should_break:
                    break
                    ...
                    if not self.emergency_stop_flag:
                        if self.paint_object == 0:
                            ...
                            if self.column_start_arm_pos is not None and self.column_start_lift_height > 0 :
                                ...
                                self.set_lift_ctrl(1, back_to_start_dist)
                                rospy.sleep(3)
                                now_time = time.time()
                                while not self.lift_stop_flag:
